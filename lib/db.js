...
// Идемпотентная миграция: если photo_url указывает на CDN Instagram (fbcdn.net),
// но локальный файл /public/uploads/ig_<id>.<ext> существует — подменяем
// photo_url на локальный путь. Решает регрессию старых записей, у которых
// в БД сохранилась внешняя ссылка вместо скачанного файла.
function migrateInstagramPhotos(db) {
  const uploadsDir = path.join(process.cwd(), 'public', 'uploads');
  if (!fs.existsSync(uploadsDir)) return;
  const rows = db.prepare("SELECT id, photo_url FROM instagram_posts").all();
  const update = db.prepare("UPDATE instagram_posts SET photo_url=? WHERE id=?");
  for (const { id, photo_url } of rows) {
    if (photo_url && photo_url.startsWith('/uploads/')) continue;
    for (const ext of ['jpg', 'png', 'webp']) {
      const filename = `ig_${id}.${ext}`;
      if (fs.existsSync(path.join(uploadsDir, filename))) {
        update.run(`/uploads/${filename}`, id);
        break;
      }
    }
  }
