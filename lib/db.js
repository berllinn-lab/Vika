...
// Запускается при каждом старте, но делает UPDATE только если данные устарели.
function migrateFooterLinks(db) {
  ...
  try {
    ...
    // Telegram: заменяем пустой/дефолтный URL
    (footer.social || []).forEach((l) => {
  ...
  const cRow = db.prepare("SELECT value FROM content WHERE key='contacts'").get();
  if (!cRow) return;
  try {
    const contacts = JSON.parse(cRow.value);
    if (contacts.telegram_url === 'https://t.me/' || contacts.telegram_url === '#') {
      contacts.telegram_url = 'https://t.me/+79114511696';
      db.prepare("UPDATE content SET value=?, updated_at=? WHERE key='contacts'")
        .run(JSON.stringify(contacts), Date.now());
    }
  } catch {}
}
